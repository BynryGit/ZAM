    ...
    if request.user.is_authenticated():
        try:
            ...
            data = {'success': 'true', 'produc_name': list1, 'product_percentage': list2, 'info': info, 'var': var,
                    ...
                    'asset_list': asset_list, 'product_amount': product_amount, 'total_sum': formatt}
...
def view_variable_details(request):
    ...
    try:
        ...
        sum = sum + int(credit_card_deb)
        variables_data = {
            'variable': 'Credit Card Debit',
            'amount': int(credit_card_deb)
        }
        variables_list.append(variables_data)

        sum = sum + int(other_loans)
        variables_data = {
            'variable': 'Other Loans',
            'amount': int(other_loans)
        }
        variables_list.append(variables_data)
        var = {'sum': int(sum)}
...
def open_new_worth(request):
    if request.user.is_authenticated():
        try:
            ...
            check_obj = UserProfile.objects.get(user_id=request.session['customer_id'])
            try:
                pension_obj = Customer_Product.objects.get(user_id=check_obj, product_id='2', row_status='Active')
                pension = pension_obj.amount
...
def view_asset_analysis(request):
    ...
    try:
        ...
        for x in range(age_gap + 2):
            ...
            new_data[pre_years] = get_year_calculationss(int(salary), percentage, x)
        ...
        for variables in cust_var_obj:
            ...
            for x in range(age_gap + 2):
                pre_years = pre_year + x
                new_data[pre_years] = get_year_calculationss(int(variables.amount), currect_percentage, x)
        ...
        saving_data = {
            ...
            pre_year: int(customer_obj.start_saving)
        }
        new_saving_data = {
            'sr_no': 2,
            'balance_sheet': 'New Saving',
            pre_year: int(temp_desc[pre_year] * 12)
        }
        investment_data = {
            'sr_no': 3,
            'balance_sheet': 'Investment returns',
            pre_year: int(get_year_calculations(saving_data[pre_year], invest_percentage))
        }
        end_saving_data = {
            'sr_no': '',
            'balance_sheet': 'End Saving',
            pre_year: int(saving_data[pre_year] + new_saving_data[pre_year] + investment_data[pre_year])
        }
...
def get_goals_details(request):
    try:
        ...
        for x in range(age_gap + 2):
            ...
            total_data[pre_years] = int(float(bullet_data[pre_years]) + float(annual_data[pre_years]))
...
def cal_cash_flow(amount, percentage, i):
    try:
        return int(amount * ((1 + percentage) ** i))
    except Exception, e:
        print 'Exception==>', e
...
def get_funding_gaps(check_obj,saving_data, total_overflow):
    try:
        ...
        for x in range(age_gap + 2):
            pre_years = pre_year + x
            funding_data[pre_years] = int(saving_data[pre_years] * 12) - int(total_overflow[pre_years])
...
def cal_cost(amount, power, percentage):
    try:
        if power == 1:
            ...
            return int(amount * ((1 + (percentage / 100)) ** power))
    except Exception, e:
        print 'Exception==>', e
